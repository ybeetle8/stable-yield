...
contract PancakePair {
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'PancakeSwap: FORBIDDEN');
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            unchecked {
                price0CumulativeLast += uint256((uint256(_reserve1) * 2**112) / _reserve0) * timeElapsed;
                price1CumulativeLast += uint256((uint256(_reserve0) * 2**112) / _reserve1) * timeElapsed;
            }
        }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = PancakeFactory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(balance0Adjusted * balance1Adjusted >= uint(_reserve0) * _reserve1 * (1000**2), 'PancakeSwap: K');
        }
