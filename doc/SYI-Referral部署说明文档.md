# SYI-Referral 推荐关系管理合约部署说明文档

## 一、合约概述

SYI-Referral 是一个独立的推荐关系管理合约,用于管理用户的推荐人和好友关系。该合约支持推荐链查询、关系绑定、批量查询等功能。
...
```bash
npx hardhat run scripts/deployReferral.js --network localhost
```

**部署输出示例**:
```
========================================
开始部署 SYI-Referral 合约...
========================================
...
========================================
部署信息已保存到: syi-referral-deployment.json
========================================
...
然后部署:
```bash
npx hardhat run scripts/deployReferral.js --network bscTestnet
```
...
import SYIReferralABI from './abis/SYIReferral.json';

// 连接到合约
const provider = new ethers.providers.JsonRpcProvider('http://localhost:8545');
const referralAddress = '0x5075F0324f90bAFDc7837E8C68C9ec6e6bCaE938';
const referral = new ethers.Contract(referralAddress, SYIReferralABI, provider);
```
...
部署成功后,合约信息会自动保存到 `syi-referral-deployment.json`:
...
└── SYIReferral.sol           # 推荐关系管理合约

scripts/
├── deployReferral.js          # 部署脚本
└── testReferral.js            # 测试脚本

syi-referral-deployment.json   # 部署信息(自动生成)
```
